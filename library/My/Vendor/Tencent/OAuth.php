...
class Http
{
    ...
    public static function request(
        $url, $params = array(), $method = 'GET', $multi = false, $extheaders = array()
    )
    {
        ...
        switch ($method) {
            ...
            case 'GET':
                ...
                if (!empty($params)) {
                    $url = $url . (strpos($url, '?') ? '&' : '?') . (is_array($params)
...
class Tencent
{
    ...
    public static function api($command, $params = array(), $method = 'GET', $multi = false)
    {
        if (isset($_SESSION['t_access_token'])) { //OAuth 2.0 方式
            ...
            $urls          = @parse_url($url);
            $sig           = SnsSign::makeSig(
                $method, $urls['path'], $params, OAuth::$client_secret . '&'
            );
            $params['sig'] = $sig;
        }
